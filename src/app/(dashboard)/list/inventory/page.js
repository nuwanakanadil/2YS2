...
export default function InventoryManagement() {
  ...
  // detect if a new image file is attached
  const hasNewImage = typeof window !== "undefined" && form.image instanceof File;
  ...
  try {
    ...
    if (method === "POST" || hasNewImage) {
      // Use FormData when creating OR when updating with a new image
      ...
      if (form.lowStockThreshold) fd.append("lowStockThreshold", String(form.lowStockThreshold));
      ...
      if (form.batchNumber) fd.append("batchNumber", form.batchNumber);
      if (hasNewImage) fd.append("image", form.image); // only append if a File

      res = await fetch(url, { method, body: fd });
    } else {
      // Update WITHOUT changing image: send JSON and OMIT the image field
