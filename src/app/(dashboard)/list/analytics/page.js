...
const CurrencyDisplay = ({ value, className = '' }) => {
  ...
  const symbol = firstDigitIndex > 0 ? str.slice(0, firstDigitIndex) : '';
  const amount = firstDigitIndex >= 0 ? str.slice(firstDigitIndex) : '';
...
const StatCard = ({ title, value, subtitle, trend, color = 'amber', icon }) => {
  ...
  return (
    <div className={`p-8 rounded-2xl border-2 shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-200 ${colorClasses[color]} min-w-[300px]`}>
      <div className="text-center">
        {icon && <div className="text-4xl mb-4">{icon}</div>}
        <h3 className="text-lg font-semibold text-gray-700 mb-3">{title}</h3>
        <div className="text-3xl font-bold mb-2">
          {/* If value looks like a currency string, render with CurrencyDisplay */}
        ...
        {trend && (
          <div className={`inline-block text-sm px-3 py-1 rounded-full font-semibold mt-3 ${trend > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {trend > 0 ? '+' : ''}{trend}%
          </div>
        )}
      </div>
...
const ReportSection = ({ title, children, onExport, icon, reportType, downloadingReport }) => (
        ...
        {onExport && (
          <div className="flex gap-3">
            <button
              ...
              disabled={downloadingReport === `${reportType}-pdf`}
              className="bg-white bg-opacity-20 hover:bg-opacity-30 disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 flex items-center gap-2 backdrop-blur-sm"
            >
              {downloadingReport === `${reportType}-pdf` ? (
            ...
            <button
              ...
              disabled={downloadingReport === `${reportType}-excel`}
              className="bg-white bg-opacity-20 hover:bg-opacity-30 disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 flex items-center gap-2 backdrop-blur-sm"
            >
              {downloadingReport === `${reportType}-excel` ? (
                <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
              ) : (
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
...
export default function AnalyticsDashboard() {
  ...
  const handleExport = async (reportType, format) => {
    ...
    try {
      ...
      const testResponse = await fetch(`${API_URL}/dashboard?period=${period}`);
      if (!testResponse.ok) {
        throw new Error('Backend API is not responding. Please make sure the backend server is running.');
      }
      
      // Fetch the file as a blob with credentials
      ...
      link.download = `${reportType}-${period}-${timestamp}.${extension}`;
                ...
                <tbody>
                  {analytics.topSelling.slice(0, 10).map((item, index) => (
                    <tr key={index} className="border-b border-amber-100 hover:bg-amber-50 transition-colors duration-150">
                      ...
                      <td className="py-4 px-6 text-right font-semibold text-green-700"><CurrencyDisplay value={formatCurrency(item.totalRevenue)} /></td>
                ...
                <tbody>
                  {analytics.leastUsed.slice(0, 10).map((item, index) => (
                    <tr key={index} className="border-b border-amber-100 hover:bg-amber-50 transition-colors duration-150">
                      ...
                      <td className="py-4 px-6 text-right font-semibold text-green-700"><CurrencyDisplay value={formatCurrency(item.currentValue)} /></td>
                  ...
                  <tbody>
                    {analytics.wasteAnalysis.items.slice(0, 10).map((item, index) => (
                      <tr key={index} className="border-b border-amber-100 hover:bg-amber-50 transition-colors duration-150">
                        ...
                        <td className="py-4 px-6 text-right font-semibold text-red-700"><CurrencyDisplay value={formatCurrency(item.totalWasteValue)} /></td>
                ...
                <div key={index} className="p-6 border-2 border-amber-100 rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 hover:shadow-lg transition-all duration-200">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-red-700 font-bold"><CurrencyDisplay value={formatCurrency(category.totalWasteValue)} /></span>
                    </div>
                    <div className="flex justify-between items-center">
