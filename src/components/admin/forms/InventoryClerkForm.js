...
// Inventory clerk: email, first/last are required. Phone/universityId optional in your schema? (Adjust if needed.)
const schema = z.object({
  email: z.string().email({ message: "Invalid email address!" }),
  firstName: z.string().min(1, { message: "First name is required!" }),
  lastName: z.string().min(1, { message: "Last name is required!" }),
  phone: z.string().optional().or(z.literal("")).transform((v) => v || ""),
  universityId: z.string().optional().or(z.literal("")).transform((v) => v || ""),
  img: z.any().optional(),
});

export default function InventoryClerkForm({ type, data, id, onDone }) {
  ...
  const onSubmit = handleSubmit(async (formData) => {
    try {
      ...
      if (type === "create") {
        ...
        alert(json.emailSent ? "Inventory clerk created and email sent" : "Inventory clerk created (email failed to send)");
        onDone?.();
        return;
      }
      ...
      if (type === "update") {
        ...
        const url = `${API_ORIGIN}/api/admin/users/${id}`; // generic users update
  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Add inventory clerk" : type === "update" ? "Update inventory clerk" : "Inventory clerk details"}
      </h1>
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <InputField label="Phone (optional)" name="phone" defaultValue={data?.phone} register={register} error={errors.phone} {...readOnlyProps} />
        <InputField label="University ID (optional)" name="universityId" defaultValue={data?.universityId} register={register} error={errors.universityId} {...readOnlyProps} />
      </div>
