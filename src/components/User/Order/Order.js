    ...
    try {
      ...
      for (const it of items) {
        const payload = {
          ...
          img: it.image,
          sessionTs,
          paymentMethod,                 // <-- fixed name & casing
        };
        ...
        // (optional) if API returns created order details:
      ...
      // If your app shows orders by session:
      // router.push(`/user/Orders/session/${sessionTs}`);
      // If your API returns an orderId above, redirect there instead:
      // router.push(`/user/Orders/${created._id}`);

      router.push("/user/Orders"); // fallback: list page
    } catch (e) {
