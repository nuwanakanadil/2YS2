...
export default function DeliveryLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch('http://localhost:5000/api/auth/delivery-login', {
      method: 'POST',
      credentials: 'include',
    ...
    if (res.ok) {
      console.log('Delivery logged in:', data);
      localStorage.setItem('deliveryEmail', JSON.stringify(data.email));
      localStorage.setItem('deliveryUserId', data.userId);
        ...
        <div className="grid lg:grid-cols-2 items-center gap-6 max-w-6xl w-full">
          ...
          <div className="border border-slate-300 rounded-lg p-6 max-w-md shadow-[0_2px_22px_-4px_rgba(93,96,127,0.2)] max-lg:mx-auto bg-[#6F4E37]">
            <form className="space-y-16" onSubmit={handleSubmit}>
              <div className="mb-12">
                <h1 className="text-white text-3xl font-semibold">Delivery Login</h1>
                <p className="text-white text-[15px] mt-6 leading-relaxed">
                  Sign in to your delivery account and start delivering orders.
                </p>
              </div>
