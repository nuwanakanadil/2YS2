...
export default function UserLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
  const { managerId, email, role, canteenId } = data;  // Retrieve canteenId from the response
  console.log('Logged in:', data);

  // Store canteenId in localStorage along with other info
  localStorage.setItem('email', JSON.stringify(email));
  localStorage.setItem('managerId', managerId);
  localStorage.setItem('canteenId', canteenId);  // Store canteenId here
