...
export default function PromotionsList() {
  ...
  const [q, setQ] = useState(""); // committed query used for fetch
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      {/* Filters + Search */}
      <div className="px-4 py-3 border-b border-gray-200 flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <form onSubmit={onSearchSubmit} className="flex gap-2 sm:ml-auto">
          <input
            type="text"
          ...
          />
          <button type="submit" className="px-3 py-1.5 text-sm rounded bg-green-600 text-white hover:bg-green-700">
            Search
          </button>
          {q ? (
            <button
        ...
        <div className="text-sm text-gray-500 sm:ml-2">Page {page} • Total {total}</div>
      </div>
